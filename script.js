...
// Implement a display function to show the first 50 iterations of Wolfram's reversible rule and then reverse the pattern
var drawReversibleRule2 = function(value, ctx){
  let pseudoImageArray = [];
  let start = Date.now();
  for (let j = 0; j < 2; j++) {
    let columns = [];
  ...
  for (let y = 0; y < 150; y++) {
    ...
    for (let x = 0; x < 200; x++) {
      ...
      if (y == 0) {
        ...
        if (y == 50) { // reverse the pattern
          if (x == 0) {
            for (let j = 0; j < 200; j++) {
              pseudoImageArray[0][j] = pseudoImageArray[0][j] ^ pseudoImageArray[1][j];
              pseudoImageArray[1][j] = pseudoImageArray[0][j] ^ pseudoImageArray[1][j];
              pseudoImageArray[0][j] = pseudoImageArray[0][j] ^ pseudoImageArray[1][j];
            }
          }
          if (pseudoImageArray[1][x] == 1) {
...
setInterval(function() {
    ...
    drawRule(number, ctx1);
    drawReversibleRule(number, ctx2);
    drawReversibleRule2(number, ctx3);
  }, 1000);
